...
|_~/.zshenv_   | *Allways*. It is read every time I USE a shell like
                 running a script.
|_~/.zprofile_ | If this is a *login shell* — i.e. I have logged in
                 via ssh or GDM and opened a shell (terminal). Running
...
|_~/.zshrc_    | If this is an *interactive shell* — i.e. I can type
